...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            //get the pointer of the fisrt space in the request line
            
            char* space1_ptr = strchr(line,' ');
            if(space1_ptr == NULL)
            {
                error(400);
                continue;
            ...
            char method[space1_ptr - line+1];
            
            strncpy(method,line,space1_ptr - line);
            
            method[space1_ptr - line] = '\0';
            ...
            {  
             ...
             // get a pointer to the beginning of the request target
               char* requestptr = strchr(space1_ptr,space1_ptr[1]);
               
               
               //get a pointer to the second space in request line
               char* space2_ptr = strchr(requestptr,' ');
               if(space2_ptr == NULL)
               {
                   error(400);
                   continue;
               ...
               // store the request target in a char array
              
               
               char request_target[space2_ptr - requestptr + 1];
               
               strncpy(request_target,requestptr,space2_ptr - requestptr);
               
               request_target[space2_ptr - requestptr]= '\0';
             ...
             //check if reguest target doesn't contain '"'
            ...
            int path_size;
            bool questionmark_exist = 0;
            // create a pointer to a '?' sign in the (request target) 
            //if it doesn't exist then absolute path = request_target
            char* question_mark = strchr(requestptr,'?');
            if(question_mark == NULL)
            {
                path_size =  strlen(request_target);
            }
            else
            { 
               path_size = question_mark - requestptr;
               questionmark_exist = 1;
                
            }
            
            
            char absolute_path[path_size+1];
            ...
            { 
             strncpy(absolute_path,requestptr,path_size);
                
            }
            absolute_path[path_size] = '\0';
            
            
            
            // check if absolute path contains "."
            ...
            // create a pointer to the beginning of the query if existed
            int size = 0;
            char* begin_query ;
            if(questionmark_exist == 1)
            {
                begin_query = strchr(question_mark,question_mark[1]);
                size = space2_ptr - begin_query;
            }
            //store the query in a char array
            char query[size + 1];
           if(size == 0)
           {
               strcpy(query,"");
           ...
           {
           strncpy(query,begin_query,size);
           }
           query[size] = '\0';
            ...
            char path[strlen(root)+path_size+1];
            strcpy(path,root);
            strcat(path,absolute_path);
            path[strlen(root)+path_size] ='\0';
            ...
            // create pointer to the path extention
            char* extension_ptr = strchr(absolute_path,'.');
            
            char extension[strlen(extension_ptr)];
            
            //extract path's extension form request target
